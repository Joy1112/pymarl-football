...
class NQLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, per_weight=None):
        ...
        for t in range(batch.max_seq_length):
            if self.args.mac == 'reg_mac':
                agent_outs = self.mac.forward(batch, t, 0, train_mode=True)
            else:
        ...
        with th.no_grad():
            ...
            for t in range(batch.max_seq_length):
                if self.args.mac == 'reg_mac':
                    target_agent_outs = self.target_mac.forward(batch, t, 0, train_mode=True)
                else:
    ...
    def save_models(self, path, mode_id=None):
        if mode_id is not None:
            ...
            if self.mixer is not None:
                th.save(self.mixer.state_dict(), "{}/mixer_{}.th".format(path, mode_id))    
            th.save(self.optimiser.state_dict(), "{}/opt_{}.th".format(path, mode_id))    
        else:
